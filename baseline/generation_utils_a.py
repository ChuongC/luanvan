...
def get_topk_evidence(image_path, 
                           ...
                           k=3):
    """
    Given an image, get the top-k evidence using CLIP similarity.
    Safe version: 
    - Decodes URL-encoded paths
    - Skips images not in image_map
    """
    # decode URL
    image_path_fixed = unquote(image_path)

    # kiểm tra tồn tại trong map
    if image_path_fixed not in image_map:
        print(f"[SKIP] Image not in embeddings map: {image_path_fixed}")
        return []

    # filter evidence
    evidence_for_image = [ev for ev in evidence if ev['image_path'] == image_path]
    evidence_index = [evidence.index(ev) for ev in evidence_for_image]

    if len(evidence_index) > k:
        image_index = int(image_map[image_path_fixed])
        sorted_evidence = sort_with_clip_score(image_index,
                                               evidence_index,
                                               image_embeddings,
                                               clip_evidence_embeddings)
...
def get_topk_demonstrations(image_path,
                                 ...
                                 k=2):
    """
    Retrieve top-k demonstrations safely:
    - Skip test image nếu không có embedding
    - Decode URL-encoded paths
    """
    # Chuẩn hóa đường dẫn
    image_path_fixed = unquote(image_path)

    # Chỉ lấy train images có label
    subset_train_index = [t for t in range(len(train)) if train[t][question].lower() != 'not enough information']
    subset_train = [train[t] for t in subset_train_index]

    train_image_idx = []
    for idx, t in zip(subset_train_index, [t['image_path'] for t in subset_train]):
        t_fixed = unquote(t)
        if t_fixed in image_map:
            train_image_idx.append((idx, int(image_map[t_fixed])))

    if image_path_fixed not in image_map:
        print(f"[SKIP] Test image not in embeddings map (demonstration): {image_path_fixed}")
        return []  # Trả về rỗng nếu test image không có embedding

    image_idx = int(image_map[image_path_fixed])
    sorted_candidates = sort_with_image_similarity(image_idx, train_image_idx, image_embeddings)
    return sorted_candidates[:k]
