...
def summarize_scores(metric_list, metric_name):
    """Tóm tắt thống kê cho một metric."""
...
if __name__=='__main__':
    ...
    parser.add_argument('--geonames_username', type=str, default="chuong", #Insert here your GeoName username. Necessary for location only
    ...
    if args.task=='source':
        ...
        summary.append(summarize_scores(rougeL, "RougeL"))
        summary.append(summarize_scores(meteor, "Meteor"))
    elif args.task=='date':
        print('EM score %s'%np.mean(exact_match))
        print('Delta score %s'%np.mean(delta))
        summary.append(summarize_scores(exact_match, "Exact Match"))
        summary.append(summarize_scores(delta, "Delta"))
    elif args.task=='location':
        print('RougeL score %s'%np.mean(rougeL))
        print('Meteor score %s'%np.mean(meteor))
        ...
        summary.append(summarize_scores(rougeL, "RougeL"))
        summary.append(summarize_scores(meteor, "Meteor"))
        summary.append(summarize_scores(codelta, "OCDelta"))
        summary.append(summarize_scores(hldelta, "HL Delta"))
    elif args.task=='motivation':
        print('RougeL score %s'%np.mean(rougeL))
        print('Meteor score %s'%np.mean(meteor))
        ...
        summary.append(summarize_scores(rougeL, "RougeL"))
        summary.append(summarize_scores(meteor, "Meteor"))
        summary.append(summarize_scores(berts, "BertS"))
    else:
        print('Invalid task name')

    # Lọc bỏ None nếu metric list rỗng
