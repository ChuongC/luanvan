...
if __name__=='__main__':
    ...
    if args.download_image:
        ...
        image_to_download = []
        for u in subset_evidence:
            if isinstance(u, list) and u:
                image_to_download.append(u[0])
            elif isinstance(u, str):
                image_to_download.append(u)
            else:
                image_to_download.append('')
    ...
    for img_path in manipulation_detection_test_image_paths:
        ...
        if len(subset) > 0:
            ...
            if 'evidence_date' in df.columns:
                df['date'] = pd.to_datetime(df['evidence_date'], unit='ms', errors='coerce')
                df = df.sort_values(by='date', na_position='last')
                sorted_evidence_by_date_index = df.index.to_list()
            else:
                sorted_evidence_by_date_index = list(range(len(subset)))
